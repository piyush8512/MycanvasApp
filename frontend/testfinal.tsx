...
export default function CanvasScreen() {
  ...
  const scale = useRef(new Animated.Value(1)).current as Animated.Value;
  const translateX = useRef(new Animated.Value(0)).current as Animated.Value;
  const translateY = useRef(new Animated.Value(0)).current as Animated.Value;
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderGrant: () => {
      // Store the current value in the offset
      scale.extractOffset();
  ...
  const renderCanvasItem = (item: (typeof canvasItems)[0]) => {
    return (
      ...
      >
        ...
        <View style={styles.itemContent}>{renderMediaContent(item)}</View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.toolbar}>
        ...
        <Text style={styles.canvasInfo}>
          Infinite canvas • pinch to zoom • drag to pan
        </Text>
      </View>
